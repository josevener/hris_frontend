...
import { PayrollConfigCreate } from "./PayrollConfigCreate";

export function PayrollConfigList() {
  ...
  return (
    <Card className="w-full max-w-5xl bg-white dark:bg-gray-800">
      <CardHeader className="flex flex-row justify-between items-center">
        <CardTitle className="text-2xl font-bold dark:text-white">Payroll Configuration Settings</CardTitle>
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Configure Payroll Cycle</DialogTitle>
            </DialogHeader>
            <PayrollConfigCreate onCreate={handleCreate} isCreating={isCreating} />
          </DialogContent>
        </Dialog>
      </CardHeader>
      <CardContent>
              ...
              configs.map((config) => (
                <TableRow key={config.id} className="dark:bg-gray-800 dark:hover:bg-gray-700">
                  ...
                  <TableCell className="dark:text-gray-200">
                    {new Date(`${config.start_year_month}-01`).toLocaleDateString("en-US", {
                      year: "numeric",
                      month: "long",
                    })}
                  </TableCell>
